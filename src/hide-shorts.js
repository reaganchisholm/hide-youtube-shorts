...
function showShortsCheck(){ 
    chrome.storage.local.get(['hys-hide-shorts-check']).then((obj) => {
        if(window.location.pathname.includes('/feed/subscriptions') == false){
            return;
        };
...
function setupButton(isHidden){
    ...
    const buttons = document.querySelector('[page-subtype="subscriptions"] #title-container');

    if(buttons == null) {
        return;
    } else if(buttons.querySelector('#hys-hide-shorts-button') == null) {
        if(isHidden == true) {
            toggleButton();
        }
...
// listen for the yt-page-date-updated event triggered by youtube when page changes
window.addEventListener('yt-page-data-updated', function () {
    console.log('yt-page-data-updated event triggered');
    console.log(window.location.pathname.includes("/feed/subscriptions"));
